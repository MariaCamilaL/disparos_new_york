...
def get_table_download_link(df): #funcion, convertir bytes y descargarlo 
    csv=df.to_csv(index= False) # descargar un dataframe en drive
    b64= base64.b64encode(csv.encode()).decode() #convertir el dataframe en bytes
...
c2.text('Victima: '+ str(top_perp_name)+'; '+ str(top_vic_num)+'%')
st.write(top_perp_name)
################---------------------------------------
...
fig = px.line(df3, x='yearmonth',y='disparos', color ='boro')
...
fig=px.pie(df2, values='incident_key', names= 'perp_sex',width=300, height=320)
...
fig=px.pie(df2, values='incident_key', names= 'vic_sex',width=300, height=320)

#### definir los colores para cada categoria TAREA
# Editar gráfica
fig.update_layout(
        title_x=0.5,
        plot_bgcolor='rgba(0,0,0,0)',
...
df2=df[df['hour'].isin([23,9])].groupby(['year','hour'])[['incident_key']].count().sort_values('incident_key').reset_index()
